...
class ShellCommandHandler(Handler):
    """This class provides a simple shell command handler."""

    SUPPORTED_PROTOCOL = ShellCommandMessage.protocol_id

    def setup(self) -> None:
        """Set up the handler."""
        self.context.logger.info("setting up ShellCommandHandler")

    def handle(self, message: Message) -> None:
        """Implement the reaction to a message."""
        message = cast(ShellCommandMessage, message)

        # recover dialogue
        shell_command_dialogues = cast(ShellCommandDialogues, self.context.shell_command_dialogues)
        shell_command_dialogue = cast(ShellCommandDialogue, shell_command_dialogues.update(message))
